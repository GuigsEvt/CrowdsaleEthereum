...
contract('BasicToken', function(accounts) {
    ...
    describe('transfer, balanceOf', async () => {
        ...
        it('should not allow transfer() of amount greater that initial funds', async () => {
            await assertJump(token.transfer(to, initialFunds + 1));
        });

        it('should not allow transfer() of amount that we do not have', async () => {
          await token.assign(owner, 0);
          await assertJump(token.transfer(to, 1));
        });
    });
    ...
    describe('approve, allowance, transferFrom', async () => {
        it('should not allow transfer() without approval', async () => {
            await assertJump(token.transferFrom(owner, spender, 1, {from: spender}));
        });
        ...
        it('should not allow approve() without resetting spender allowance to 0', async () => {
            ...
            await assertJump(token.approve(spender, allowedAmount + 1));
        });
        ...
        it('should not allow transfer() over approve() amount', async () => {
            ...
            await assertJump(token.transferFrom(owner, to, allowedAmount, {from: spender}));

        });

        it('should not changed balance if transferFrom is called but not allowed', async () => {
          await token.approve(spender, allowedAmount - 1);
        ...
        it('should not changed allowance if transferFrom is called but not allowed', async () => {
          await token.approve(spender, allowedAmount - 1);
